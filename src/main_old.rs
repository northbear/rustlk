...
fn tcp_server_process(bind_ip: &SocketAddr) -> (mpsc::Sender<String>, mpsc::Receiver<String>) {
        ...
        loop {
            match listener.accept() {
                Ok((mut socket, addr)) => {
                    ...
                    thread::spawn(move || {
                        // let fp = from_process.borrow_mut();
                        loop {
                            match from_process.try_recv() {
                                Ok(message) => {
...
fn tcp_client_process(ip: &SocketAddr) -> (mpsc::Sender<String>, mpsc::Receiver<String>) {
...
//    let (to_peer, from_peer) = tcp_server_process("0.0.0.0:8888"); 
    ...
    let (to_peer, from_peer) = match conf.mode() {
        Mode::Server(bind) => tcp_server_process(&bind), 
        Mode::Client(ip)   => tcp_client_process(&ip),
        Mode::Usage        => {
            println!("{}", conf.usage());
            return
        },
